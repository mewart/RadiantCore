...
class BaseVocoderConfig(BaseTrainingConfig):
    ...
    optimizer_params: dict = field(
        default_factory=lambda: {"betas": [0.8, 0.99], "weight_decay": 0.0}
    )
...
class BaseGANVocoderConfig(BaseVocoderConfig):
    ...
    lr_scheduler_gen_params: dict = field(
        default_factory=lambda: {"gamma": 0.999, "last_epoch": -1}
    )
    ...
    steps_to_start_discriminator = (
        0  # start training the discriminator after this number of steps.
    )
