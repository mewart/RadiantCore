...
class Synthesizer(object):
    ...
    def _set_tts_speaker_file(self):
        ...
        if (
            hasattr(self.tts_config, "use_speaker_embedding")
            ...
            if self.tts_config.use_d_vector_file:
                self.tts_speakers_file = (
                    ...
                    else self.tts_config["d_vector_file"]
                )
                ...
                self.tts_speakers_file = (
                    self.tts_speakers_file
                    if self.tts_speakers_file
                    else self.tts_config["speakers_file"]
                )
    ...
    def _load_tts(
        ...
        if stats_path and (not os.path.isfile(stats_path)):
            stats_path = os.path.join(
                os.path.dirname(tts_checkpoint), os.path.split(stats_path)[1]
            )
    ...
    def _load_vocoder(self, model_file: str, model_config: str, use_cuda: bool) -> None:
        ...
        if stats_path and (not os.path.isfile(stats_path)):
            stats_path = os.path.join(
                os.path.dirname(model_file), os.path.split(stats_path)[1]
            )
    ...
    def tts(
        self, text: str, speaker_idx: str = "", speaker_wav=None, style_wav=None
    ) -> List[int]:
