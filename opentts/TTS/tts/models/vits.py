...
from TTS.tts.layers.vits.networks import (
    PosteriorEncoder,
...
class Vits(BaseTTS):
    ...
    def __init__(self, config: Coqpit):
        ...
        if args.init_discriminator:
            self.disc = VitsDiscriminator(
                use_spectral_norm=args.use_spectral_norm_disriminator
            )
    ...
    def init_multispeaker(self, config: Coqpit, data: List = None):
        ...
        if config.use_speaker_embedding and not config.use_d_vector_file:
            ...
            self.emb_g = nn.Embedding(
                config.num_speakers, config.speaker_embedding_channels
            )
        # init d-vector usage
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            logp1 = torch.sum(-0.5 * math.log(2 * math.pi) - logs_p, [1]).unsqueeze(
                -1
            )  # [b, t, 1]
            logp2 = torch.einsum("klm, kln -> kmn", [o_scale, -0.5 * (z_p ** 2)])
            logp3 = torch.einsum("klm, kln -> kmn", [m_p * o_scale, z_p])
            logp4 = torch.sum(-0.5 * (m_p ** 2) * o_scale, [1]).unsqueeze(
                -1
            )  # [b, t, 1]
        ...
        if self.args.use_sdp:
            ...
            loss_duration = torch.sum(
                (log_durations - attn_log_durations) ** 2, [1, 2]
            ) / torch.sum(x_mask)
    ...
    def inference(self, x, aux_input={"d_vectors": None, "speaker_ids": None}):
        ...
        if self.args.use_sdp:
            logw = self.duration_predictor(
                x, x_mask, g=g, reverse=True, noise_scale=self.inference_noise_scale_dp
            )
        else:
        ...
        logs_p = torch.matmul(attn.transpose(1, 2), logs_p.transpose(1, 2)).transpose(
            1, 2
        )

        z_p = (
            m_p + torch.randn_like(m_p) * torch.exp(logs_p) * self.inference_noise_scale
        )
    ...
    def train_step(
        ...
        if optimizer_idx == 0:
            ...
            with autocast(enabled=False):  # use float32 for the criterion
                loss_dict = criterion[optimizer_idx](
                    outputs["scores_disc_real"], outputs["scores_disc_fake"],
                )
