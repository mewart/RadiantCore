...
class BaseTacotron(BaseTTS):
    def __init__(self, config: Coqpit):
        ...
        if self.gst and self.use_gst:
            self.decoder_in_features += (
                self.gst.gst_embedding_dim
            )  # add gst embedding dim
            self.gst_layer = None
    ...
    def get_aux_input(self, **kwargs) -> Dict:
        ...
        # setup d_vector
        d_vector = (
            self.speaker_manager.get_d_vectors_by_speaker(
                self.speaker_manager.speaker_names[0]
            )
    ...
    def _coarse_decoder_pass(self, mel_specs, encoder_outputs, alignments, input_mask):
        ...
        if T % self.coarse_decoder.r > 0:
            ...
            mel_specs = torch.nn.functional.pad(
                mel_specs, (0, 0, 0, padding_size, 0, 0)
            )
    ...
    def compute_speaker_embedding(self, speaker_ids):
        ...
        if hasattr(self, "speaker_project_mel") and speaker_ids is not None:
            self.embedded_speakers_projected = self.speaker_project_mel(
                self.embedded_speakers
            ).squeeze(1)
    ...
    def compute_gst(self, inputs, style_input, speaker_embedding=None):
        ...
        if isinstance(style_input, dict):
            ...
            gst_outputs = self.gst_layer(
                style_input, speaker_embedding
    ...
    def _add_speaker_embedding(outputs, embedded_speakers):
        embedded_speakers_ = embedded_speakers.expand(
