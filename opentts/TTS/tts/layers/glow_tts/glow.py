...
class InvConvNear(nn.Module):
    ...
    def __init__(
        self, channels, num_splits=4, no_jacobian=False, **kwargs
        ...
        if LooseVersion(torch.__version__) < LooseVersion("1.9"):
            w_init = torch.qr(
                torch.FloatTensor(self.num_splits, self.num_splits).normal_()
            )[0]
        else:
    ...
    def forward(
        ...
        x = (
            x.permute(0, 1, 3, 2, 4)
            .contiguous()
            .view(b, self.num_splits, c // self.num_splits, t)
        )
        ...
        if reverse:
            ...
            if self.no_jacobian:
                ...
                logdet = (
                    torch.logdet(self.weight) * (c / self.num_splits) * x_len
                )  # [b]
...
class CouplingBlock(nn.Module):
    ...
    def forward(
        self, x, x_mask=None, reverse=False, g=None, **kwargs
    ):  # pylint: disable=unused-argument
        """
        Shapes:
            - x: :math:`[B, C, T]`
