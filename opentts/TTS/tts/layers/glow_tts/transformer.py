...
class RelativePositionMultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        ), " [!] channels should be divisible by num_heads."
        # class attributes
        ...
        if rel_attn_window_size is not None:
            ...
            emb_rel_k = nn.Parameter(
                torch.randn(n_heads_rel, rel_attn_window_size * 2 + 1, self.k_channels)
                * rel_stddev
            )
        ...
        if mask is not None:
            ...
            if self.input_length is not None:
                block_mask = (
                    ...
                    .triu(-1 * self.input_length)
                    .tril(self.input_length)
                )
