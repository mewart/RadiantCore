...
class GravesAttention(nn.Module):
    ...
    def init_layers(self):
        torch.nn.init.constant_(
            self.N_a[2].bias[(2 * self.K) : (3 * self.K)], 1.0
        )  # bias mean
    ...
    def init_states(self, inputs):
        ...
        self.attention_weights = torch.zeros(inputs.shape[0], inputs.shape[1]).to(
            inputs.device
        )
    ...
    def forward(self, query, inputs, processed_inputs, mask):
        ...
        # attention weights
        phi_t = g_t.unsqueeze(-1) * (
            1 / (1 + torch.sigmoid((mu_t.unsqueeze(-1) - j) / sig_t.unsqueeze(-1)))
        )
...
class OriginalAttention(nn.Module):
    ...
    def init_forward_attn(self, inputs):
        ...
        self.alpha = torch.cat(
            [torch.ones([B, 1]), torch.zeros([B, T])[:, :-1] + 1e-7], dim=1
        ).to(inputs.device)
    ...
    def get_location_attention(self, query, processed_inputs):
        ...
        energies = self.v(
            torch.tanh(processed_query + processed_attention_weights + processed_inputs)
        )
    ...
    def apply_forward_attention(self, alignment):
        ...
        alpha = (
            (1 - self.u) * self.alpha + self.u * fwd_shifted_alpha + 1e-8
        ) * alignment
        ...
        if not self.training and self.forward_attn_mask:
            ...
            for b in range(alignment.shape[0]):
                ...
                alpha[b, (n[b] - 2)] = (
                    0.01 * val[b]
                )  # smoothing factor for the prev step
    ...
    def forward(self, query, inputs, processed_inputs, mask):
        ...
        if self.norm == "softmax":
            ...
            alignment = torch.sigmoid(attention) / torch.sigmoid(attention).sum(
                dim=1, keepdim=True
            )
        else:
...
class MonotonicDynamicConvolutionAttention(nn.Module):
    ...
    def forward(self, query, inputs, processed_inputs, mask):
        ...
        prior_filter = F.conv1d(
            F.pad(self.attention_weights.unsqueeze(1), (self.prior_filter_len - 1, 0)),
        ...
        dynamic_filter = dynamic_filter.view(
            query.size(0), self.dynamic_filter_dim, -1
        ).transpose(1, 2)
        # compute static filters
        static_filter = self.static_filter_conv(
            self.attention_weights.unsqueeze(1)
        ).transpose(1, 2)
        alignment = (
            self.v(
                torch.tanh(
                    ...
                    + self.dynamic_filter_layer(dynamic_filter)
                )
            ).squeeze(-1)
            + prior_filter
        )
