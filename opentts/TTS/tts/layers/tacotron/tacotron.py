...
class CBHG(nn.Module):
    ...
    def __init__(
        ...
        self.gru = nn.GRU(
            gru_features, gru_features, 1, batch_first=True, bidirectional=True
        )
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        prenet_dim = (
            frame_channels * self.memory_size
            if self.use_memory_queue
            else frame_channels
        )
        self.prenet = Prenet(
            prenet_dim, prenet_type, prenet_dropout, out_features=[256, 128]
        )
    ...
    def _init_states(self, inputs):
        ...
        if self.use_memory_queue:
            self.memory_input = torch.zeros(1, device=inputs.device).repeat(
                B, self.frame_channels * self.memory_size
            )
        else:
            self.memory_input = torch.zeros(1, device=inputs.device).repeat(
                B, self.frame_channels
            )
        # decoder states
    ...
    def decode(self, inputs, mask=None):
        ...
        for idx, decoder_rnn in enumerate(self.decoder_rnns):
            self.decoder_rnn_hiddens[idx] = decoder_rnn(
            ...
            if self.memory_size > self.r:
                ...
                self.memory_input = torch.cat(
                    [
                        new_memory,
                        self.memory_input[
                            :, : (self.memory_size - self.r) * self.frame_channels
                        ].clone(),
                    ],
                    dim=-1,
                )
                ...
                self.memory_input = new_memory[
                    :, : self.memory_size * self.frame_channels
                ]
        else:
    ...
    def inference(self, inputs):
        ...
        while True:
            ...
            t += 1
            if t > inputs.shape[1] / 4 and (
                stop_token > 0.6 or attention[:, -1].item() > 0.6
            ):
                break
