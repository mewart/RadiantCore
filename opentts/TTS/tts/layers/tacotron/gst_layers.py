...
class GST(nn.Module):
    ...
    def __init__(
        ...
        self.style_token_layer = StyleTokenLayer(
            num_heads, num_style_tokens, gst_embedding_dim, embedded_speaker_dim
        )
...
class StyleTokenLayer(nn.Module):
    ...
    def __init__(
        self, num_heads, num_style_tokens, gst_embedding_dim, d_vector_dim=None
    ):
        super().__init__()
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key):
        ...
        queries = torch.stack(
            torch.split(queries, split_size, dim=2), dim=0
        )  # [h, N, T_q, num_units/h]
        keys = torch.stack(
