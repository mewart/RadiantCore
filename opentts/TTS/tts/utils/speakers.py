...
class SpeakerManager:
    ...
    def __init__(
        ...
        if data_items:
            self.speaker_ids, self.speaker_names, _ = self.parse_speakers_from_data(
                self.data_items
            )
    ...
    def set_d_vectors_from_file(self, file_path: str) -> None:
        ...
        self.clip_ids = list(
            set(sorted(clip_name for clip_name in self.d_vectors.keys()))
        )
    ...
    def get_d_vectors_by_speaker(self, speaker_idx: str) -> List[List]:
        ...
        """
        return [
            x["embedding"] for x in self.d_vectors.values() if x["name"] == speaker_idx
        ]

    def get_mean_d_vector(
        self, speaker_idx: str, num_samples: int = None, randomize: bool = False
    ) -> np.ndarray:
        ...
        if num_samples is None:
            ...
            assert (
                len(d_vectors) >= num_samples
            ), f" [!] speaker {speaker_idx} has number of samples < {num_samples}"
            if randomize:
    ...
    def init_speaker_encoder(self, model_path: str, config_path: str) -> None:
        ...
        self.speaker_encoder.load_checkpoint(
            config_path, model_path, eval=True, use_cuda=self.use_cuda
        )
        ...
        if restore_path:
            ...
            if c.use_d_vector_file:
                ...
                if not os.path.exists(speakers_file):
                    print(
                        "WARNING: speakers.json was not found in restore_path, trying to use CONFIG.d_vector_file"
                    )
                ...
                not c.use_d_vector_file
            ):  # restor speaker manager with speaker ID file.
                speaker_ids_from_data = speaker_manager.speaker_ids
                speaker_manager.set_speaker_ids_from_file(speakers_file)
                assert all(
                    speaker in speaker_manager.speaker_ids
                    for speaker in speaker_ids_from_data
