...
def plot_alignment(
    alignment, info=None, fig_size=(16, 10), title=None, output_fig=False
):
    ...
    alignment_ = (
        alignment_.astype(np.float32) if alignment_.dtype == np.float16 else alignment_
    )
...
def plot_spectrogram(spectrogram, ap=None, fig_size=(16, 10), output_fig=False):
    ...
    spectrogram_ = (
        spectrogram_.astype(np.float32)
        if spectrogram_.dtype == np.float16
        else spectrogram_
    )
    if ap is not None:
...
def visualize(
    ...
    if CONFIG.use_phonemes:
        ...
        text = sequence_to_phoneme(
            seq, tp=CONFIG.characters if "characters" in CONFIG.keys() else None
        )
        print(text)
    plt.yticks(range(len(text)), list(text))
    plt.colorbar()
