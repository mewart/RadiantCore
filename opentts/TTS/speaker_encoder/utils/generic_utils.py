...
class AugmentWAV(object):
    def __init__(self, ap, augmentation_config):
        ...
        if "rir" in augmentation_config.keys():
            ...
            print(
                f" | > Using RIR Noise Augmentation: with {len(self.rir_files)} audios instances"
            )
...
def setup_model(c):
    if c.model_params["model_name"].lower() == "lstm":
        ...
        model = ResNetSpeakerEncoder(
            input_dim=c.model_params["input_dim"], proj_dim=c.model_params["proj_dim"]
        )
    return model


def save_checkpoint(
    model, optimizer, criterion, model_loss, out_path, current_step, epoch
):
...
def save_best_model(
    model, optimizer, criterion, model_loss, best_loss, out_path, current_step
):
