...
def download_and_extract(directory, subset, urls):
    ...
    try:
        for url in urls:
            ...
            subprocess.call(
                "wget %s --user %s --password %s -O %s"
                % (url, USER["user"], USER["password"], zip_filepath),
                shell=True,
            )
        ...
        if ".zip" not in zip_filepath:
            ...
            subprocess.call(
                "cat %s* > %s.zip" % (zip_filepath, zip_filepath), shell=True
            )
            zip_filepath += ".zip"
...
def convert_audio_and_make_label(input_dir, subset, output_dir, output_file):
    ...
    df = pandas.DataFrame(
        data=files,
        columns=["wav_filename", "wav_length_ms", "speaker_id", "speaker_name"],
    )
